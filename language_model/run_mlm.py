...
from utils import Config, Monitor, cuda_setup
from models import BERT_KG, Regression, BertConfigCustom, RobertaConfigCustom, BertRegularizationTrainer
...
class ModelArguments:
    ...
    knowledge_base: Optional[str] = field(
        default="freebase",
        metadata={"help": "Use this KB node_dict for regression regularization process [freebase, wordnet]."},
    )
...
def main():
    ...
    if data_args.dataset_name is not None:
        raw_datasets = load_dataset(
            ...
            cache_dir=data_args.data_folder + "datasets/" + str(data_args.dataset_name)
        )
    ...
    model_mlm.resize_token_embeddings(len(tokenizer))
    ...
    if data_args.line_by_line:
        if os.path.exists(
                data_args.data_folder + "datasets/tokenized_datasets.pt"):
            logger.info("Load tokenized dataset: " + data_args.data_folder + "datasets/tokenized_datasets.pt")
            tokenized_datasets = torch.load(data_args.data_folder + "datasets/tokenized_datasets.pt")
        else:
    ...
    config_custom = Config(json_config)
    regression_model = Regression(768, 200, 200)
    regression_model.info()

    if model_args.knowledge_base:
        GRAPH_REGULARIZATION = model_args.knowledge_base
