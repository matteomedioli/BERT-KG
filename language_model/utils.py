...
def generate_node_dict_gat():
    synset_lookup = torch.load("/data/medioli/wordnet/synset_lookup_node_id.pt")
    gat_embeddings = torch.load("/data/medioli/wordnet/entities_gat_sept.pt")
    node_dict = {}
    for node_id, synset_name in synset_lookup.items():
        node_dict[synset_name] = gat_embeddings[node_id]
    torch.save(node_dict, "/data/medioli/wordnet/node_dict_gat_sept.pt")
    return node_dict


def generate_wikidata_from_freebase():
    D = {}
    freebase_file = open("freebase_entities.txt")
    lines = freebase_file.readlines()
    for i, line in enumerate(lines):
        freebaseid = line.split("\t")[0]
        ...
        SELECT DISTINCT ?item ?itemLabel WHERE {
          SERVICE wikibase:label { bd:serviceParam wikibase:language "[AUTO_LANGUAGE]". }
          {
            SELECT DISTINCT ?item WHERE {
              ...
              ?statement0 (ps:P646) "[FREEBASEID]".
            }
          }
        }
        '''
        query = query.replace("[FREEBASEID]", freebaseid)
        ...
        if data["results"]["bindings"]:
            D[i] = data["results"]["bindings"][0]["itemLabel"]["value"]
        else:
            D[i] = freebaseid
        print(D)

    torch.save(D, "freebase_wikidata_dict.pt")


def retrieve_wikidata(text):
    for word in text:
